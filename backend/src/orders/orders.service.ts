...
function calcDeliveryCost(method: DeliveryMethod, subtotal: number): number {
...
export class OrdersService {
  ...
  async confirmReceived(userId: string, orderId: string) {
    ...
    if (order.status !== 'awaiting_pickup') {
      throw new BadRequestException('Подтвердить получение можно только для заказа, готового к выдаче')
    }
